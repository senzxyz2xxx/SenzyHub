...
makeToggle(farmPage, "Auto Vote Next", "โหวต next หลังจบด่าน", 4, function()
    ...
    while states["Auto Vote Next"] do
        if game.ReplicatedStorage:GetAttribute("RoundEndTimer") then
            ...
            -- รอจนกว่า Timer จะหาย (ด่านใหม่เริ่ม) ถึงจะเริ่มเช็คโหวตครั้งต่อไป
            repeat task.wait(1) until not game.ReplicatedStorage:GetAttribute("RoundEndTimer") or not states["Auto Vote Next"]
        end
        task.wait(1)
    end
end)
