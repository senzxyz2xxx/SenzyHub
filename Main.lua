...
makeButton(scroll, "Unit Dex Reward", "claim unit dex reward", 5, function()
    RF.UnitDex.ClaimUnitReward:InvokeServer()
end)
        ...
        while states["Auto Collect"] do
            ...
            if #parts == 0 then
                ...
                for _, part in ipairs(parts) do
                    ...
                    task.wait(1.0) -- [ปรับให้ช้าลง] รอให้ตัวละครโหลดตำแหน่งเสร็จ
                    
                    pcall(function()
                        -- เริ่มกดปุ่ม E
                        game:GetService("VirtualInputManager"):SendKeyEvent(true, Enum.KeyCode.E, false, game)
                        
                        task.wait(0.8) -- กดค้างไว้ 0.8 วินาที
                        
                        -- ปล่อยปุ่ม E
                        game:GetService("VirtualInputManager"):SendKeyEvent(false, Enum.KeyCode.E, false, game)
                    end)

                    -- ตรวจสอบ ProximityPrompt รอบๆ
                    for _, obj in ipairs(workspace:GetDescendants()) do
                        if obj:IsA("ProximityPrompt") then
                            local p = obj.Parent
                            if p and p:IsA("BasePart") then
                                if (p.Position - root.Position).Magnitude < 12 then
                    ...
                    task.wait(1.5) -- [ปรับให้ช้าลง] พักสักครู่ก่อนไปหีบถัดไป เพื่อความปลอดภัย
                end
            end
        end
    end
...
makeToggle(scroll, "Auto Claim Quest", "claim quest วนอัตโนมัติ", 9,
    function()
        while states["Auto Claim Quest"] do
            pcall(function() RF.Quests.ClaimQuest:InvokeServer() end)
            task.wait(3)
        end
    end
)
